...
Planned Additions or fixes: 
    *Attempt to add a feature to upload/download multiple files. Use multiprocessing if you can to simultaniously do it.
    *Add a feature when * <-- is sent into the send or download function then download/upload all the files in that directory.
...
def send(filename):#This function is used to send files or .zip files to the ftp.
    ...
    time = (endtime-startime).total_sds()
    file.close()
...
def connect(ipAddress, port=21): # The connect code was being repeted twice, so i made a function.
    ...
    try:
        ...
        print("Login Successful.\nConnected to: %s\nConnection Speed: %s b/s\nWelcome Message: %s" % (ipAddress,bytes,welcome)) # This line is fixed. (Date: 22 March 2017, Day: Wednesday, Time: 08:27:35)
    except Exception as e:
        print(e)
...
#====================Connection=======================
        ...
        try:
            ...
            print("Err run connect to disconnect")
#================End Connection=======================
...
#====================End Misc=========================
        ...
        print("{} not found, type help for command list".format(command))
