...
Update Notes:
    ...
    -Ok Now the connect function had a place where it tests connection speed and in that I use the Test.py on ftp to do this. But now i started a server on ubuntu and So that means if I need to prevent myself from deleting the file I need to hide and when i do that the name changes from "Test.py" to ".Test.py" So I changed that in this script. And so when using the windows server you use the other script when using the ubuntu server you use this script. (Date: 1 April 2017, Day: Saturday, Time: 13:42:07)
