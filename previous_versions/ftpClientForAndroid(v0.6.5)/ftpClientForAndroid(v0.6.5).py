...
Planned Additions or fixes:
    *If connection to ftp is false do not allow authentication.
    *A make command. that Makes a folder on FTP.
    *Organize all command outputs.
    *zipfolder function has an issue. Check and fix.
    *Need a delete directory command because, delete command only deletes files.
...
while True:
    ...
    if command[0] == "lls":
        ...
        #For the moment this is a pass. But later, add the developer name (my name) and version and some disclaimers.(This is absolutly point less by the way.
