...
def download(filename, extention):#This function is to retrieve or download a file from the ftp.
    ...
    try:#Added this, because when downloading a file that does not exist program crashes. (Date: 7 March 2017, Day: Tuesday, Time: 14:42:28)
        ...
        print("%s.%s Not Found." % (filename, extention))
        file.close
        os.remove(filename + "." + extention)#The created file was still there, so this deletes the file.
        return False
    endtime = datetime.datetime.now()
    time = (endtime-startime).total_seconds()
    file.close()
