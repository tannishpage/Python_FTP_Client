...
def zipfolder(foldername, target_dir):#This function is used to Zip a folder and then send it over to the ftp
    if foldername == '*':
        dir_zip_folder([x for x in os.listdir(os.getcwd()) if os.path.isdir(x)])
    else:    
...
def dir_zip_folder(folderlist, target_dir=os.getcwd()):#This function is new, its objective is to zip all folders in a directory and start transfering all of them.
    foldernamezip = []
